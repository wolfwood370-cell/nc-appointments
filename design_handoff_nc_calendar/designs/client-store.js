...
(function () {
  ...
  function load() {
    ...
    if (!window.__ncSynced) {
      window.__ncSynced = true;
      try {
        syncBookings();
      } catch (e) {}
    }
  ...
  // ---- derived ----
  function curBlock() {
    return state.block.n;
  }
  function inCurBlock(s) {
    return (s.block || state.block.n) === state.block.n;
  }
  function poolFor(type) {
    ...
    return {
      ...
      base: p.total,
      credits: extra,
      left: Math.max(0, p.total + extra - used),
    };
  }
  ...
  function byId(id) {
    return (
      state.sessions.filter(function (s) {
        return s.id === id;
      })[0] || null
    );
  }
  ...
  function blockStats() {
    var b = state.block;
    var done = state.sessions.filter(function (s) {
      return s.status === "done" && inCurBlock(s);
    }).length;
    var booked = state.sessions.filter(function (s) {
      return s.status === "upcoming" && inCurBlock(s);
    }).length;
  ...
  function pendingFeedback() {
    return (
      past().filter(function (s) {
        return s.status === "done" && (s.feedback === null || s.feedback === undefined);
      })[0] || null
    );
  }
  ...
  function syncBookings() {
    var all = [];
    try {
      all = JSON.parse(localStorage.getItem(BOOKINGS_KEY) || "[]");
    } catch (e) {
      all = [];
    }
    all = all.filter(function (b) {
      return b.clientId !== CLIENT_ID;
    }); // drop this client's old rows
  ...
  function addCredits(type, n) {
    ...
    state.credits[type] = (state.credits[type] || 0) + n;
    save();
  }
  function resetAll() {
    try {
  ...
  function scenario(name) {
    ...
    if (name === "empty") {
      ...
      for (var i = 0; i < size; i++) {
        var day = 3 + i;
        var dd = new Date(2026, 5, day); // giugno
        sess.push({
          id: "eb" + i,
  ...
  function notifications() {
    ...
    if (n && !n.confirmed) {
      list.push({
        ...
        sub: TYPES[n.type].label + " · " + relativeDay(n.date).toLowerCase() + " alle " + n.time,
      });
    }
    var bs = blockStats();
    if (bs.open > 0)
      list.push({
        ...
        sub: "Hai " + bs.open + " sessioni da prenotare entro il " + bs.deadline,
      });
    ...
    if (lastB)
      list.push({
        ...
        sub: "Peso " + lastB.weight + " kg · massa " + lastB.muscle + " kg (" + lastB.m + ")",
      });
    ["PT", "BIA", "TEST"].forEach(function (k) {
      ...
      if (p.left <= 0 && p.total > 0)
        list.push({
          ...
          color: "#8E24AA",
          title:
            "Pool " + (k === "PT" ? "Sessioni PT" : k === "BIA" ? "BIA" : "Test") + " esaurito",
    ...
    if (pf)
      list.push({
        ...
        color: "#4361ee",
        title: "Com'è andata?",
        sub: "Lascia un feedback sulla sessione del " + fmtShort(pf.date),
      });
    var read = readSet();
    list.forEach(function (x) {
      x.unread = read.indexOf(x.id) < 0;
    });
    return list;
  }
  function unreadCount() {
    return notifications().filter(function (x) {
      return x.unread;
    }).length;
  }
  function markAllRead() {
    var ids = notifications().map(function (x) {
      return x.id;
    });
    try {
  ...
  function relativeDay(dateISO) {
    ...
    if (diff > 1 && diff < 7)
      return DOW[d.getDay()].charAt(0).toUpperCase() + DOW[d.getDay()].slice(1) + " " + d.getDate();
    return d.getDate() + " " + MSHORT[d.getMonth()];
  }
  function endTime(time, dur) {
    var p = time.split(":");
    var m = +p[0] * 60 + +p[1] + (dur || 60);
